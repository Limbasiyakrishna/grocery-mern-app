...
const BottomNav = () => {
    ...
    return (
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-emerald-100 px-4 py-2 flex justify-between items-center z-[60] shadow-[0_-4px_12px_-1px_rgba(16,185,129,0.1)]">
            <NavLink
                to="/"
            ...
            <NavLink
                to="/cart"
                className={({ isActive }) => `flex flex-col items-center gap-1 relative transition-colors ${isActive ? 'text-emerald-600' : 'text-slate-400'}`}
            >
                <div className="relative">
                    ...
                    {cartCount() > 0 && (
                        <span className="absolute -top-1 -right-2 bg-emerald-600 text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold ring-2 ring-white">
                            {cartCount()}
                        </span>
                    )}
                </div>
